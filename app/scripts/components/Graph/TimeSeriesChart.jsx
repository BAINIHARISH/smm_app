...
TimeSeriesChart.defaultProps = {
  ...
  showTooltip(data) {
    ...
    const evt = window.event || d3.event;
    const targetTopDiff = (evt.pageY || evt.clientY) - evt.currentTarget.getBoundingClientRect().top;
    ...
    let top = lineRect.top + targetTopDiff,left = lineRect.left + 10;

    const clientRect = this.tooltip.node().getBoundingClientRect();
    if(top + clientRect.height > screenHeight){
      top -= clientRect.height;
    }
    if(left + clientRect.width > screenWidth){
      left -= (clientRect.width + 20);
    }
  ...
  hideTooltip() {
    this.tooltip.style({visibility: 'hidden',top: 0, left:0});
